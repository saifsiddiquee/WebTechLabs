...
session_unset();
    ...
    $string = "$user||$pass||$repass||$email||$phone||$fname";

    if(!strstr($file, "$string"))
    {
    	$myFile = "log.txt";
    	$fp = fopen($myFile, 'a') or die("can't open file");
        $stringData = "$user||$pass||$repass||$email||$phone||$fname\n";
    ...
    {
    	echo "Sorry the username: $username and emal: $email is already taken. please use different";
    }	
